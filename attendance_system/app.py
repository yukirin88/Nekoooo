...
def calculate_average(sleep_times):
    ...
    from app import evaluate_sleep  # 循環インポートを避けるために関数内でインポート
...
@login_required
def average_sleep():
    period = request.args.get('period', 'all')  # デフォルトは全期間
    ...
    with get_db_connection() as conn:
        ...
        for i in range(len(data)):
            if data[i]['action'] == 'sleep':
                sleep_start = datetime.fromisoformat(data[i]['timestamp'])
            elif data[i]['action'] == 'wake_up' and sleep_start:
                wake_time = datetime.fromisoformat(data[i]['timestamp'])
                ...
                sleep_date = datetime.fromisoformat(data[i]['timestamp']).date()
                sleep_times.append({
                    'date': sleep_date,
        ...
        return render_template('average_sleep.html',
                              ...
                              overall_avg=overall_avg,  # 全期間の平均を追加
...
def calculate_overall_average(sleep_times):
    ...
    evaluation = evaluate_sleep(avg_sleep) if len(sleep_times) >= 3 else None

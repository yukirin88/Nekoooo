...
DATABASE_PATH = os.path.join(os.environ.get('RENDER_DATA_DIR', os.path.dirname(os.path.abspath(__file__))), DATABASE_FILE)
TIMEZONE = pytz.timezone('Asia/Tokyo')
MAX_BACKUPS = 5

# ====== データベース関連関数 ======
def ensure_db_directory_exists(db_path=DATABASE_PATH):
...
def is_db_empty(db_path=DATABASE_PATH):
    """DBが空かどうかをチェックする関数"""
...
def validate_github_credentials():
    return all([GITHUB_TOKEN, GITHUB_USERNAME, GITHUB_REPOSITORY])

def backup_database():
    """GitHubにDBをバックアップする関数（新しい実装）"""
    if not validate_github_credentials() or not os.path.exists(DATABASE_PATH) or is_db_empty(DATABASE_PATH):
        print("GitHub認証情報不足 or DBファイルなし/空。バックアップスキップ。")
        return
    timestamp = datetime.now(TIMEZONE).strftime('%Y-%m-%d_%H-%M-%S')
    backup_filename = f"backup_{timestamp}_{DATABASE_FILE}"
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_url = f"https://x-access-token:{GITHUB_TOKEN}@github.com/{GITHUB_USERNAME}/{GITHUB_REPOSITORY}.git"
            try:
                subprocess.run(["git", "clone", "--branch", "db-backup", repo_url, tmpdir], check=True, capture_output=True)
            except subprocess.CalledProcessError:
                # db-backupブランチが存在しない場合は新規作成
                subprocess.run(["git", "clone", repo_url, tmpdir], check=True)
                subprocess.run(["git", "checkout", "-b", "db-backup"], cwd=tmpdir, check=True)
            
            # バックアップファイルをコピー
            shutil.copy2(DATABASE_PATH, os.path.join(tmpdir, backup_filename))
            
            # 古いバックアップを削除（最新MAX_BACKUPS件のみ保持）
            backups = sorted([f for f in os.listdir(tmpdir) if f.startswith("backup_")], reverse=True)
            ...
            subprocess.run(["git", "commit", "-m", f"Auto backup: {backup_filename}"], cwd=tmpdir, check=True)
            subprocess.run(["git", "push", "origin", "db-backup"], cwd=tmpdir, check=True)
            print(f"バックアップ成功: {backup_filename}")
...
def restore_database():
    """GitHubからDBをリストアする関数"""
    if not validate_github_credentials():
        print("GitHub認証情報不足。リストアスキップ。")
        return False
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo_url = f"https://x-access-token:{GITHUB_TOKEN}@github.com/{GITHUB_USERNAME}/{GITHUB_REPOSITORY}.git"
            try:
                ...
                print("db-backupブランチが見つかりません。リストア不可。")
                return False
                
            backups = sorted([f for f in os.listdir(tmpdir) if f.startswith("backup_")], reverse=True)
            if backups:
                shutil.copy2(os.path.join(tmpdir, backups[0]), DATABASE_PATH)
                print(f"リストア成功: {backups[0]}")
                return True
            print("リストア可能なバックアップなし。")
            return False
    except Exception as e:
        print(f"リストア失敗: {e}")
        return False
    ...
    with get_db_connection() as conn:
        ...
        try:
            ...
            columns = [
                ...
                ('records', 'is_private', 'INTEGER DEFAULT 0')
            ]
...
scheduler.add_job(backup_database, 'interval', hours=1, next_run_time=datetime.now() + timedelta(minutes=1))
scheduler.start()
...
def calculate_average(sleep_times):
    """全期間の平均睡眠時間を計算"""
    if not sleep_times:
        return {'avg_hours': 0, 'avg_minutes': 0, 'evaluation': None, 'avg_duration': 0.0}
...
# calculate_overall_averageの実装（未定義だったため追加）
def calculate_overall_average(sleep_times):
    return calculate_average(sleep_times)
...
def round_decimal(value, places=1):
    """小数点以下の桁数を指定して四捨五入する"""
    if value is None:
        return None
...
def index():
    ...
    user_message = session.pop(f"user_{session['user_id']}_message", None)
        ...
        with get_db_connection() as conn:
            ...
            if existing_like:
                ...
                conn.execute(
                    ...
                    (session['user_id'], record_id, jst_now().isoformat())
                )
...
def average_sleep_view():
    try:
        ...
        with get_db_connection() as conn:
            sleep_times = get_sleep_times(conn, session['user_id'])
        ...
        flash("平均睡眠時間の計算中にエラーが発生しました。", "danger")
        return redirect(url_for('index'))
            ...
            if sleep_data:
                ...
                for row in sleep_data:
                    if row['action'] == 'sleep' and not sleep_start:
                        ...
                        # 評価メッセージを決定
                        sleep_evaluation = evaluate_sleep(sleep_duration)
                        break
...
def sleep_data():
    try:
        ...
        with get_db_connection() as conn:
            ...
            if period == 'daily':
                data = [{'date': s['date'].isoformat(), 'duration': s['duration']} for s in sleep_times]
                data.sort(key=lambda x: x['date'])
            elif period == 'weekly':
                weekly_avg = calculate_weekly_average(sleep_times)
                data = [{'period': w['period'], 'avg_duration': w['avg_duration']} for w in weekly_avg]
            elif period == 'monthly':
                monthly_avg = calculate_monthly_average(sleep_times)
                data = [{'period': m['period'], 'avg_duration': m['avg_duration']} for m in monthly_avg]
            else:
                return jsonify({'error': '無効な期間パラメータです'}), 400
